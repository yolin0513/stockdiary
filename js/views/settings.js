...
function backupSection() {
  ...
  return h('section', { class: 'card', dataset: { card: 'backup' } },
    ...
    // **這一段是換機與救援的關鍵，而且很容易踩到。**
    // iPhone 把「加到主畫面的 App」與 Safari 當成兩個不同的網站來存資料
    // （STATUS「外部環境造成的」那一節記過）。在 Safari 匯出、到主畫面 App 匯入，
    // 會發現資料不見 —— 而使用者會以為是 App 壞了、或以為自己備份過了。
    // 沒有帳號也沒有雲端，這個檔案就是唯一的路。
    h('p', { class: 'warn sm', dataset: { note: 'storageSplit' } },
      '這個 App 沒有帳號也沒有雲端，資料只存在這台手機的瀏覽器裡。'),
    h('p', { class: 'warn sm' },
      'iPhone 上「加到主畫面的 App」與 Safari 算兩個地方，資料不共用 —— '
      + '在哪一邊匯出，就要在哪一邊匯入。平常用主畫面的 App，就在主畫面的 App 裡匯出。'),
    h('p', { class: 'muted sm' },
      '換手機、清掉瀏覽器資料、或把 App 從主畫面移除，資料就沒了，只能靠這個檔案救回來。'),
    exportBtn,

    h('h3', { class: 'sub-title' }, '匯入'),
    h('p', { class: 'muted sm' },
      '匯入是取代：現在這台裝置上的持股、異動、計畫、除權息、設定會被備份檔的內容蓋掉，'
