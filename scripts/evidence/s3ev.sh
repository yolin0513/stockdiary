...
echo; echo "【對照：沒改壞，新版照常產報告】"
reset; run assertaudit.mjs; echo "  exit=$CODE"; grep -E "✓|fmttest|來自" "$OUT" | head -6
case_tiny; case_crash; case_fail
echo; echo "【突變（commit 之後、在複本裡）：--only S3：】"
reset; git diff --quiet || { echo "複本不乾淨"; exit 1; }
rm -f "$T/m.txt"; node scripts/mutationtest.mjs --only 'S3：' > "$T/m.txt" 2>&1; echo "  exit=$?"; grep -E "S3：|mutationtest：" "$T/m.txt"
reset
