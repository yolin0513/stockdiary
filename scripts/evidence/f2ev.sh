#!/usr/bin/env bash
# 【本質一次性，保留供重做】用途：範圍外第 2 件的修正前（livecheck 沒打 FMTQIK）｜比較：77b026d vs c0bc4db｜數字在：docs/EVIDENCE_檢查器修補.md「**第 2 件已做**」｜不用守：對明確的舊 commit 量一次，舊版不會再變；新版由常設測試守著（證據檔「盤點」一節）
