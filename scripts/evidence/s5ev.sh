...
for f in assertaudit gatescan taptest; do
...
echo; echo "【assertaudit：npm test 鏈上多一支新測試 newtest，清單沒收】"
reset
cp scripts/fmttest.mjs scripts/newtest.mjs
node "$SP/patch.mjs" scripts/newtest.mjs "done('fmttest');" "done('newtest');" || exit 1
node "$SP/patch.mjs" package.json "node scripts/fmttest.mjs && " "node scripts/fmttest.mjs && node scripts/newtest.mjs && " || exit 1
grep -q "node scripts/newtest.mjs" package.json || { echo "情境沒造成"; exit 1; }
rm -f "$OUT"; node scripts/assertaudit-old.mjs fmt > "$OUT" 2>&1; echo "  修正前（assertaudit fmt）：exit=$?"; grep -E "newtest|來自|不完整" "$OUT"
rm -f "$OUT"; node scripts/assertaudit.mjs fmt > "$OUT" 2>&1; echo "  修正後（assertaudit fmt）：exit=$?"; grep -E "newtest|清單|停下" "$OUT"
