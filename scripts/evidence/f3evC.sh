#!/usr/bin/env bash
# 【本質一次性，保留供重做】用途：範圍外第 3 件：§5.12 兩道一起拿掉｜比較：2b4aa09 上改壞兩道｜數字在：docs/EVIDENCE_檢查器修補.md「**第 3、4 件已做**」｜不用守：對明確的舊 commit 量一次，舊版不會再變；新版由常設測試守著（證據檔「盤點」一節）
...
echo "  只拿掉錯誤卡那一道"
git checkout -q -- scripts/upgradecheck.mjs
...
run node scripts/upgradecheck.mjs; echo "  exit=$CODE"; grep -E "upgradecheck：" "$OUT"
echo "  只拿掉標題那一道"
git checkout -q -- scripts/upgradecheck.mjs
patch scripts/upgradecheck.mjs "const pageOk = (x) => x.landed && x.text > 20 && !x.errorCard && x.errs.length === 0;" "const pageOk = (x) => x.text > 20 && !x.errorCard && x.errs.length === 0;"
run node scripts/upgradecheck.mjs; echo "  exit=$CODE"; grep -E "upgradecheck：" "$OUT"
echo "  兩道一起拿掉"
git checkout -q -- scripts/upgradecheck.mjs
patch scripts/upgradecheck.mjs "const pageOk = (x) => x.landed && x.text > 20 && !x.errorCard && x.errs.length === 0;" "const pageOk = (x) => x.text > 20 && x.errs.length === 0;"
run node scripts/upgradecheck.mjs; echo "  exit=$CODE"; grep -E "upgradecheck：" "$OUT"
reset
