...
const MUTATIONS = [
  ...
  },
  {
    ...
    test: 'datatest',
    expect: "6488 不支援報價",
    alsoRed: [" 不支援報價","所有非上市的代號都判成不支援","「支援報價」的判斷有對照組"],
    alsoRedWhy: "「支援報價」只看是不是上市；突變讓上櫃也回 true，單檔、逐檔（模板訊息「<代號> 不支援報價」）、全部非上市、對照組四條看的是同一個判斷。",
  },
  {
    ...
    test: 'datatest',
    expect: "三個「照常交易」的日子被挑出來，沒有被當成休市",
    alsoRed: ["「照常交易」的標記判斷有對照組","三個「照常交易」的日子都在交易日裡"],
    alsoRedWhy: "「照常交易」的標記判斷改壞，對照組、挑出來的日子、交易日清單三條都從它來。",
  },
  {
    name: '交易日不排除週末',
    ...
    expect: "交易日裡沒有週六週日",
    alsoRed: ["2026 年推出 "],
    alsoRedWhy: "週末也算進交易日，全年交易日數跟著超出範圍。",
  },
  {
  ...
  },
  {
    ...
    test: 'throttletest',
    expect: "每一段間隔都 >= ",
    alsoRed: ["沒有任何一段偷跑","五個請求至少花了 ","同時發出時每一段間隔還是 >= "],
    alsoRedWhy: "間隔整個拿掉：逐段間隔、有沒有偷跑、總耗時、同時發出的情境，四條量的是同一個間隔。",
  },
  {
    name: '拿掉單次開頁的請求上限',
    ...
    replace: '',
    test: 'throttletest',
    expect: "超過上限時丟 BudgetExceededError",
  },
  {
    name: '請求不排隊，同時發出去',
  ...
  // ---- M2：除權息與股利 ----
  {
    ...
    test: 'dividendtest',
    expect: "這些筆的 cashPerShare 全部是 null，沒有一筆是 0",
    alsoRed: ["有配息待公告的筆：","每一筆都帶得出原因（cashNote）"],
    alsoRedWhy: "待公告被當成 0 元：待公告的筆數（前提）變成 0，原因欄也跟著不見。",
  },
  {
    ...
    test: 'dividendtest',
    expect: "96.699523 捨去成 96.69",
    alsoRed: ["參考價逐筆對得上：","六筆全部一致"],
    alsoRedWhy: "捨去改成四捨五入，逐筆對帳與「六筆全部一致」看的是同一批參考價。",
  },
  {
    ...
    test: 'dividendtest',
    expect: "剛好 20,000 元要扣 422 元",
    alsoRed: ["實收 19,568 元","門檻判斷有對照組"],
    alsoRedWhy: "剛好 20,000 元那一筆不再扣補充保費：扣額、實收、門檻對照組看的是同一筆。",
  },
  {
    name: '自動扣費關閉時還是扣了匯費',
    ...
    test: 'dividendtest',
    expect: "關閉自動扣費時，實收就是 500 元（一毛都不扣）",
    alsoRed: ["沒有匯費"],
    alsoRedWhy: "關閉自動扣費時還扣匯費，實收與「沒有匯費」看的是同一筆。",
  },
  {
    ...
    test: 'dividendtest',
    expect: "1000 股 × 0.04999999 = 49.99999 → 加 49 股，不是 50",
    alsoRed: [" 股以現金找零 —— 不會自己進位成 50 股"],
    alsoRedWhy: "配股率被進位，配出的股數與找零的零股是同一次計算。",
  },
  {
    name: '累積股利把未確認的也算進去',
    ...
    test: 'dividendtest',
    expect: "三筆算得出金額",
    alsoRed: ["總計 11,490 元","2026 年 8,490 元","分年度，新的在前","2330 累積 3,000 ＋ 3,490 = 6,490 元","總計是 null","年度也是 null","算得出金額的筆數是 0"],
    alsoRedWhy: "未確認的也被加總：總計、分年度、逐檔累積，以及「全部未確認」那組的 null，都從同一個加總來。",
  },
  {
    ...
    test: 'dividendtest',
    expect: "2330 累積 3,000 ＋ 3,490 = 6,490 元",
    alsoRed: ["總計 11,490 元","2026 年 8,490 元","分年度，新的在前"],
    alsoRedWhy: "改用預估金額，那一筆的金額變了，總計、那一年、分年度排序都跟著變。",
  },
  {
    name: '除權息事件不把參考價存下來',
  ...
  },
  {
    ...
    test: 'dividendtest',
    expect: "配息還沒公告（null）→ null，**不當成 0 元配息**",
    alsoRed: ["而且不是「參考價等於前收」","配股率是 null → null","增資配股率是 null → null","「推導得出來嗎」的判斷有對照組"],
    alsoRedWhy: "「推導得出來嗎」把 null 當成 0：配息、配股率、增資配股率三種 null 與對照組都從同一個判斷來。",
  },
  {
    name: '結算時不去推導參考價',
  ...
  // ---- M3：定期定額與平均成本 ----
  {
    ...
    test: 'plantest',
    expect: "9/6 是週日，順延並記下原本的日期",
    alsoRed: ["三個扣款日","9/16 沒有順延","9/26 順延到 9/29","to 之後不算"],
    alsoRedWhy: "下界比對錯了，範圍裡的扣款日清單整個不一樣，每一個日子的斷言一起紅。",
  },
  {
    name: '順延不限制跳躍距離',
    ...
    replace: '  return next;',
    test: 'plantest',
    expect: "下一個交易日在兩個月後 → 回 null",
    alsoRed: ["「跳太遠就是缺口」的判斷有對照組"],
    alsoRedWhy: "拿掉跳躍距離的限制，情境本身與它的對照組看的是同一個判斷。",
  },
  {
    name: '扣款不扣手續費就估股數',
    ...
    test: 'plantest',
    expect: "可用金額 4,992.875 元（手續費 7.125）",
    alsoRed: ["餘額 8.075 元","（對照）填錯單位的話會變成 39 股"],
    alsoRedWhy: "不扣手續費，可用金額、餘額與對照組的股數都從同一個可用金額算出來。",
  },
  {
    ...
    test: 'plantest',
    expect: "股數是 null",
    alsoRed: ["不是 0 —— 0 股代表「買不到一股」"],
    alsoRedWhy: "拿不到收盤價被估成 0 股：「是 null」與「不是 0」看的是同一個值。",
  },
  {
    name: '定期定額扣款自動確認',
 ...
 * 解析一份 RSS／Atom`,
  ...
  },
  {
    ...
    expect: "只有一檔算得出市值",
    alsoRed: ["三檔被排除"],
    alsoRedWhy: "算不出市值的被當成 0 算進去，算得出的檔數與被排除的檔數是同一件事。",
  },
  {
    ...
    test: 'concentrationtest',
    expect: "分母只有那一檔",
    alsoRed: ["只有一檔算得出市值","三檔被排除"],
    alsoRedWhy: "不支援報價的也算進去，分母、算得出的檔數、被排除的檔數一起變。",
  },
  {
    name: '佔比改用浮點數算',
    ...
    test: 'concentrationtest',
    expect: "每一份都是 33.3%",
    alsoRed: ["其他電子業 400,000 / 2,240,000 ＝ 17.8%","它就是 100%"],
    alsoRedWhy: "佔比的算法改成浮點數，三份均分、單一產業、單一檔各自的佔比一起變。",
  },
  {
  ...
  },
  {
    ...
    replace: '    if (false) {',
    test: 'plantest',
    expect: "把百分比當比例填（0.1425）會被擋",
    alsoRed: ["費率 1 不合理（那是 100%）","檢查器有對照組"],
    alsoRedWhy: "上限放寬，填百分比、填 1、檢查器的對照組都靠同一道上限。",
  },
  {
    name: 'ETF 歸到「產業未知」',
  ...
  },
  {
    ...
    test: 'datatest',
    expect: "中秋節休市",
    alsoRed: ["元旦休市","元旦隔天照常交易"],
    alsoRedWhy: "多年格式讀不懂，那一年的每一個休市日與照常交易日一起錯。",
  },
  {
    name: 'A1：日曆快用完時總覽不出聲',
  ...
  },
  {
    ...
    test: 'datatest',
    expect: "59 天還不算過期",
    alsoRed: ["剛好 60 天也還不算（門檻是「超過」）","沒過期時沒有那句話","門檻是 60 天"],
    alsoRedWhy: "門檻改成 0，59 天、60 天、沒過期時的提示、門檻本身四條看的是同一個數。",
  },
  {
  ...
  // ---- PB：四類自查的每一個分支各有自己的樣本（2026-09-25，補充說明（十一）第 3 點）：拿掉一個分支，只紅它的樣本 ----
  { name: 'PB：(a) 不認 gh 權杖', why: '四種金鑰格式拿掉一種，對照組以前照樣全過。', file: 'scripts/precheck.mjs',
    find: 'gh[pousr]_[A-Za-z0-9]{20,}|', replace: '', test: 'controltest', expect: 'gh 權杖' },
  { name: 'PB：(a) 不認 sk-ant 金鑰', why: '四種金鑰格式拿掉一種，對照組以前照樣全過。', file: 'scripts/precheck.mjs',
    find: '|sk-ant-[A-Za-z0-9_-]{16,}', replace: '', test: 'controltest', expect: 'sk-ant 金鑰' },
  { name: 'PB：(a) 不認 AIza 金鑰', why: '四種金鑰格式拿掉一種，對照組以前照樣全過。', file: 'scripts/precheck.mjs',
    find: '|AIza[A-Za-z0-9_-]{20,}', replace: '', test: 'controltest', expect: 'AIza 金鑰' },
  { name: 'PB：(a) 不認 xox 權杖', why: '四種金鑰格式拿掉一種，對照組以前照樣全過。', file: 'scripts/precheck.mjs',
    find: '|xox[baprs]-[A-Za-z0-9-]{10,}', replace: '', test: 'controltest', expect: 'xox 權杖' },
  { name: 'PB：(b) noreply@github.com 不再排除', why: '正常的 noreply 作者欄被當成外洩，每次推送都被擋（排除的分支以前沒有反例）。', file: 'scripts/precheck.mjs',
    find: '(anthropic\\.com|github\\.com)', replace: '(anthropic\\.com)', test: 'controltest', expect: 'noreply@github.com 不算' },
  { name: 'PB：(b) noreply@anthropic.com 不再排除', why: 'Claude 的 Co-Authored-By 信箱被當成外洩。', file: 'scripts/precheck.mjs',
    find: '(anthropic\\.com|github\\.com)', replace: '(github\\.com)', test: 'controltest', expect: 'noreply@anthropic.com 不算' },
  { name: 'PB：(b) users.noreply.github.com 不再排除', why: 'GitHub 帶編號的 noreply 信箱被當成外洩。', file: 'scripts/precheck.mjs',
    find: '|@users\\.noreply\\.github\\.com', replace: '', test: 'controltest', expect: 'users.noreply.github.com 不算' },
  { name: 'PB：(d) 磁碟機路徑不認反斜線', why: 'Windows 的「磁碟機代號＋冒號＋反斜線」路徑漏掉。', file: 'scripts/precheck.mjs',
    find: '[A-Za-z]:[\\\\/]|', replace: '[A-Za-z]:[/]|', test: 'controltest', expect: '磁碟機（反斜線）' },
  { name: 'PB：(d) 磁碟機路徑不認斜線', why: '「磁碟機代號＋冒號＋斜線」寫法的路徑漏掉。', file: 'scripts/precheck.mjs',
    find: '[A-Za-z]:[\\\\/]|', replace: '[A-Za-z]:[\\\\]|', test: 'controltest', expect: '磁碟機（斜線）' },
  { name: 'PB：(d) 不認 /home/', why: 'Linux 家目錄路徑漏掉。', file: 'scripts/precheck.mjs',
    find: '(home|Users)', replace: '(Users)', test: 'controltest', expect: '/home/ 家目錄' },
  { name: 'PB：(d) 不認 /Users/', why: 'macOS 家目錄路徑漏掉（以前只有 (c) 類的樣本碰得到 /Users/，那是另一支搜尋式）。', file: 'scripts/precheck.mjs',
    find: '(home|Users)', replace: '(home)', test: 'controltest', expect: '/Users/ 家目錄' },

  // ---- GB：壞寫法掃描每一條規則的每一個分支（2026-09-25，補充說明（十一）第 3 點）：拿掉一個分支，那條規則的對照組要紅 ----
  { name: "GB：pipe 關鍵指令不認 precheck", why: "pipe 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "(precheck|piiscan|gatepush|", replace: "(piiscan|gatepush|", test: 'gatescan', expect: "pipe" },
  { name: "GB：pipe 關鍵指令不認 piiscan", why: "pipe 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "(precheck|piiscan|gatepush|", replace: "(precheck|gatepush|", test: 'gatescan', expect: "pipe" },
  { name: "GB：pipe 關鍵指令不認 gatepush", why: "pipe 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "(precheck|piiscan|gatepush|", replace: "(precheck|piiscan|", test: 'gatescan', expect: "pipe" },
  { name: "GB：pipe 關鍵指令不認 git -C <路徑>", why: "pipe 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "git\\s+(?:-C\\s+\\S+\\s+)?(?:", replace: "git\\s+(?:", test: 'gatescan', expect: "pipe" },
  { name: "GB：pipe 關鍵指令不認 git push", why: "pipe 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "(?:push|fetch|ls-remote|log|diff|show)", replace: "(?:fetch|ls-remote|log|diff|show)", test: 'gatescan', expect: "pipe" },
  { name: "GB：pipe 關鍵指令不認 git fetch", why: "pipe 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "(?:push|fetch|ls-remote|log|diff|show)", replace: "(?:push|ls-remote|log|diff|show)", test: 'gatescan', expect: "pipe" },
  { name: "GB：pipe 關鍵指令不認 git ls-remote", why: "pipe 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "(?:push|fetch|ls-remote|log|diff|show)", replace: "(?:push|fetch|log|diff|show)", test: 'gatescan', expect: "pipe" },
  { name: "GB：pipe 關鍵指令不認 git log", why: "pipe 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "(?:push|fetch|ls-remote|log|diff|show)", replace: "(?:push|fetch|ls-remote|diff|show)", test: 'gatescan', expect: "pipe" },
  { name: "GB：pipe 關鍵指令不認 git diff", why: "pipe 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "(?:push|fetch|ls-remote|log|diff|show)", replace: "(?:push|fetch|ls-remote|log|show)", test: 'gatescan', expect: "pipe" },
  { name: "GB：pipe 關鍵指令不認 git show", why: "pipe 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "(?:push|fetch|ls-remote|log|diff|show)", replace: "(?:push|fetch|ls-remote|log|diff)", test: 'gatescan', expect: "pipe" },
  { name: "GB：format-no-meta 不認 -p", why: "format-no-meta 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "])-p([", replace: "])-NOPE([", test: 'gatescan', expect: "format-no-meta" },
  { name: "GB：format-no-meta 不認 --patch", why: "format-no-meta 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "|--patch|", replace: "|", test: 'gatescan', expect: "format-no-meta" },
  { name: "GB：format-no-meta 不認 git show", why: "format-no-meta 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "|\\bshow\\b/", replace: "/", test: 'gatescan', expect: "format-no-meta" },
  { name: "GB：absent-assert 不認 !existsSync", why: "absent-assert 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "!\\s*(fs\\.)?existsSync\\(|", replace: "", test: 'gatescan', expect: "absent-assert" },
  { name: "GB：absent-assert 不認 [ ! -f ]", why: "absent-assert 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "|\\[\\s*!\\s*-[efsd]\\s|", replace: "|", test: 'gatescan', expect: "absent-assert" },
  { name: "GB：absent-assert 不認「不見」", why: "absent-assert 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "|不見|", replace: "|", test: 'gatescan', expect: "absent-assert" },
  { name: "GB：absent-assert 不認「已刪」", why: "absent-assert 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "|已刪/", replace: "/", test: 'gatescan', expect: "absent-assert" },
  { name: "GB：plus3-header 不認 '^\\+\\+\\+' 樣式", why: "plus3-header 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "|\\^\\\\\\+\\\\\\+\\\\\\+|", replace: "|", test: 'gatescan', expect: "plus3-header" },
  { name: "GB：plus3-header 不認 '+++' 字面", why: "plus3-header 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "|['\"]\\+\\+\\+['\"]/", replace: "/", test: 'gatescan', expect: "plus3-header" },
  { name: "GB：empty-catch 不認帶參數的 catch (e) {}", why: "empty-catch 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "catch\\s*(\\([^)]*\\))?\\s*\\{", replace: "catch\\s*\\{", test: 'gatescan', expect: "empty-catch" },
  { name: "GB：shell-backslash 不認 grep", why: "shell-backslash 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "(grep|sed|", replace: "(sed|", test: 'gatescan', expect: "shell-backslash" },
  { name: "GB：shell-backslash 不認 sed", why: "shell-backslash 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "|sed|", replace: "|", test: 'gatescan', expect: "shell-backslash" },
  { name: "GB：shell-backslash 不認 node -e", why: "shell-backslash 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "|node\\s+-e)", replace: ")", test: 'gatescan', expect: "shell-backslash" },
  { name: "GB：env-switch 不認 X=\"${X:-…}\"", why: "env-switch 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: "/\\b([A-Z_][A-Z0-9_]*)=\"\\$\\{\\1:-/.test(l) || ", replace: "", test: 'gatescan', expect: "env-switch" },
  { name: "GB：env-switch 不認 process.env", why: "env-switch 規則少了這個分支，對照組以前可能照樣全過（每個分支要有自己的合成樣本）。", file: 'scripts/gatescan.mjs',
    find: " || /process\\.env\\.(?!(USERNAME|USER)\\b)[A-Za-z_]/.test(l)", replace: "", test: 'gatescan', expect: "env-switch" },

  // ---- PP：第五類語境樣式每個分支一條突變（2026-09-25，補充說明（十一）第 3 點）----
  { name: "PP：指認詞不認「使用者的」", why: '語境樣式少了這個分支，以前的對照組（一個樣本）照樣全過。', file: 'scripts/piiscan.mjs',
    find: "/使用者的|他的|你的|Yolin 的|我的定期定額/", replace: "/他的|你的|Yolin 的|我的定期定額/", test: 'controltest', expect: "指認詞「使用者的」" },
  { name: "PP：指認詞不認「他的」", why: '語境樣式少了這個分支，以前的對照組（一個樣本）照樣全過。', file: 'scripts/piiscan.mjs',
    find: "/使用者的|他的|你的|Yolin 的|我的定期定額/", replace: "/使用者的|你的|Yolin 的|我的定期定額/", test: 'controltest', expect: "指認詞「他的」" },
  { name: "PP：指認詞不認「你的」", why: '語境樣式少了這個分支，以前的對照組（一個樣本）照樣全過。', file: 'scripts/piiscan.mjs',
    find: "/使用者的|他的|你的|Yolin 的|我的定期定額/", replace: "/使用者的|他的|Yolin 的|我的定期定額/", test: 'controltest', expect: "指認詞「你的」" },
  { name: "PP：指認詞不認「Yolin 的」", why: '語境樣式少了這個分支，以前的對照組（一個樣本）照樣全過。', file: 'scripts/piiscan.mjs',
    find: "/使用者的|他的|你的|Yolin 的|我的定期定額/", replace: "/使用者的|他的|你的|我的定期定額/", test: 'controltest', expect: "指認詞「Yolin 的」" },
  { name: "PP：指認詞不認「我的定期定額」", why: '語境樣式少了這個分支，以前的對照組（一個樣本）照樣全過。', file: 'scripts/piiscan.mjs',
    find: "/使用者的|他的|你的|Yolin 的|我的定期定額/", replace: "/使用者的|他的|你的|Yolin 的/", test: 'controltest', expect: "指認詞「我的定期定額」" },
  { name: "PP：代號不認 00 開頭", why: '語境樣式少了這個分支，以前的對照組（一個樣本）照樣全過。', file: 'scripts/piiscan.mjs',
    find: "/\\b00\\d{2,4}\\b|\\b[1-9]\\d{3}\\b/", replace: "/\\b[1-9]\\d{3}\\b/", test: 'controltest', expect: "代號 00 開頭" },
  { name: "PP：代號不認四位數", why: '語境樣式少了這個分支，以前的對照組（一個樣本）照樣全過。', file: 'scripts/piiscan.mjs',
    find: "/\\b00\\d{2,4}\\b|\\b[1-9]\\d{3}\\b/", replace: "/\\b00\\d{2,4}\\b/", test: 'controltest', expect: "代號四位數" },
  { name: "PP：金額不認沒有千分位的", why: '語境樣式少了這個分支，以前的對照組（一個樣本）照樣全過。', file: 'scripts/piiscan.mjs',
    find: "|\\d+\\s*元(整|起)?/", replace: "/", test: 'controltest', expect: "金額（沒有千分位）" },

  // ---- BB：buildtest 判斷函式的分支（2026-09-25，補充說明（十一）第 3 點）----
  { name: 'BB：目錄雜湊不看子目錄', why: '輸出目錄裡子目錄的檔被改寫，「data/ 沒被動到」照樣成立。', file: 'scripts/buildtest.mjs',
    find: '      if (e.isDirectory()) walk(p);', replace: '      if (e.isDirectory()) continue;', test: 'buildtest', expect: '（對照）目錄雜湊：子目錄裡改一個字也會變' },
  { name: 'BB：堆疊的判斷只認帶括號的那一種', why: '模組頂層崩掉時堆疊那一行不帶括號，會被當成「沒吐堆疊」、判成設計好的停下。', file: 'scripts/buildtest.mjs',
    find: ':\\d+:\\d+\\)?\\s*$/', replace: ':\\d+:\\d+\\)\\s*$/', test: 'buildtest', expect: '（對照）堆疊的判斷：不帶括號的那一種' },
