...
> 最後更新：2026-09-11。**M0–M3 完成；M4 起尚未開發。Worker 尚未建立（M5）。**
...
測試現況：14 支測試、約 830 條斷言，`npm run mutationtest` 用 **55 條突變**逐一證明關鍵斷言改壞會紅。
...
### 除權息參考價的自算備援：**驗證已完成，等使用者決定要不要開**

**現況（線上行為）**：TWT49U 的日期參數無效（`FEASIBILITY.md` §10.8），參考價只能在除權息
當天抓到並存起來。事後回補到的除權息日標 `exNoRef`，那一檔**不計入**當日損益，畫面明講
「尚未取得參考價」。**App 目前不會自己算參考價。**

**驗證做完了**（2026-09-11）。原本以為要等配對樣本，結果 TWT48U 的預告表會把**剛過去幾天**
的事件也一起列出來（9/9 起），所以當天就抓到 6 筆重疊。公式
...
對 6 筆（息 4、權 1、權息 1）算出來的數字與證交所公布值**完全一致**，包含最難的
5906 台南-KY（現金股利 0.2 ＋ 配股率 0.05000001 → 39.38）與 9906 欣巴巴（純配股 → 41.05）。

- 實作：`js/dividend.js` 的 `refPriceFromForecast()`（全整數運算；浮點數在這裡真的會差一分錢，
  例如 `1 − 0.34` 在 double 上捨去成 0.65）。**目前沒有任何地方呼叫它。**
- 樣本：`scripts/fixtures/refprice-pairs.json`，由 `npm run livecheck` 自動捕捉並累積。
- 測試：`scripts/dividendtest.mjs` 逐筆核對，並有對照組（把現金股利改成 0 → 每一筆都算不出公布值）。

**還沒做的決定**：要不要在拿不到 TWT49U 時用它當備援。
- 開的話：回補到的除權息日就有基準價，損益曲線不會缺洞；但畫面上會出現一個不是證交所
  直接給的數字，**必須標示「參考價為依證交所公式試算」**。
- 不開（現況）：那一檔那一天不計入，畫面說明原因。

**使用者已知道這個取捨，決定尚未做出。在使用者說可以之前不要接上去。**
樣本每跑一次 livecheck 就會多累積幾筆，決定拖久一點反而更有把握。
