...
1. **「今日觀察」拿不到持股的當日漲跌％。**
   `PLAN` §7.2 的輸入包含「持股代號＋名稱＋產業＋**當日漲跌％**」，但 `js/views/news.js`
   組給 `insight.buildPrompt` 的持股只有 `{code, name, industry}` ——
   `buildUserContent` 因此對每一檔都寫「當日無報價」。
   **不是錯誤輸出，是少了脈絡**：模型看不到哪一檔今天在動。
2. **「今日觀察」拿不到大盤漲跌％。**
   `buildPrompt` 有 `marketChangePct` 參數，但沒有任何呼叫端傳它。
   要傳得先有第 3 項。
3. **加權指數（大盤）整個沒有實作。**
   `FEASIBILITY` §1 列了 `MI_INDEX` 端點（ACAO `*`，可前端直打）但從未接上。
   影響：第 2 項、以及「我 vs 大盤」這類 v2 功能。
4. **除權息只涵蓋上市。** 見 A，但若日後要支援上櫃，解析器要另寫（欄位名不同）。
