...
- 用 **Write 工具**直接寫檔，不要經過 shell heredoc（本 App `CLAUDE.md` 第 9 條同一個理由）。
...
- **`CLAUDE.md` 既有的每一行一字不動**——包括與共用慣例重複的條文、第 5 條凍結區、第 6、7 條紅線。重複就讓它重複；去重是以後另外拍板的事。
- **`docs/STATUS.md` 的既有內容不動。** 只准照你平常收尾的做法更新檔頭「最後更新」與「目前進行中／交接」節（記一筆這次導入）。**不要動任何「N 支」「N 條突變」的數字**、不要動「上線前檢查清單」與「元件慣例」兩節——`doctest` 釘著它們，而這次沒有新增任何測試，數字本來就不該變。
- **不准跑 `npm test`。** 它的鏈尾是 `mutationtest`，會暫時改寫工作目錄的原始碼（**包含凍結檔** `js/avgcost.js`、`js/settle.js`、`js/money.js`），中途中斷就會留下被改過的凍結檔。也不要跑 `mutationtest` 的任何形式。
- 不改 `package.json`、`README.md`、`scripts/`、`js/`、`workers/`、`data/`、`sw.js`、`index.html`，不新增任何測試、不新增 npm script。
...
| V5 | **公開前自查**（本 repo 是 public）。**commit 之後、push 之前**，把這次 commit 的全部新增行抽出來（`git show HEAD --format= --unified=0`，取 `+` 開頭、排除 `+++` 的行），查四類：(a) 金鑰或 token；(b) email；(c) 你這台機器的本機使用者名稱（從環境變數取，不要把它寫進任何檔案）；(d) 磁碟機代號開頭的路徑與家目錄路徑。搜尋式用 Grep 工具或寫成檔案的腳本，不要經過 `node -e`／heredoc | 四類全部零命中。**每一類都要先有對照組命中，才算數**：(d) 用同一個搜尋式去搜本 repo 的 `CLAUDE.md`（統籌者 2026-09-19 實測有 1 處）與 `docs/STATUS.md`（2 處）；(b) 去搜 `git log -1 --format=%ae` 的輸出；(c) 去搜一個你臨時造的、含該名稱的字串；(a) 沒有可以合理去讀的真實樣本，用臨時拼出來的假 token 樣式（例如把 `gh` 與 `p_` 加二十幾個英數字拼起來）。對照組沒命中就代表那個搜尋式壞了，那一類不算過。**把四個搜尋式、對照組命中在哪、目標零命中，寫進回報；回報送出後先不要 push，等 Dispatch 回覆再 push** |
| V6 | `npm run doctest`、`npm run shelltest` | 兩支全綠（新增一個 `docs/` 檔、`CLAUDE.md` 多七行，不該弄紅它們） |

## commit 與回報

- 一個 commit，訊息以 `docs(conventions):` 開頭，例如 `docs(conventions): 導入共用慣例 v1（副本＋CLAUDE.md 指向）`。
- **先 commit、跑 V5、回報、等回覆，然後才 `git push`**（本 repo 的 `main` 有設 upstream，用你平常的方式推）。推完不用 curl 確認版本（沒有 bump）、不用跑 `sweep`。
- 本 App `CLAUDE.md` 第 10 條說「做完一批就部署、回報版本號」——這次 docs-only、沒有版本號，回報 commit hash 即可。
- 回報請包含：
  1. commit hash（本機的；此時還沒 push）。
  2. V1–V6 逐項結果（貼指令輸出的關鍵行）。分開寫「實測驗證過的」與「推論的」。
  3. **衝突清單**：把副本逐條讀過，列出共用條文與本 App `CLAUDE.md`／`docs/STATUS.md` 既有規則**實質衝突**的地方（不是措辭不同）；沒有就明講沒有。已知可以順便看的兩處：共用 §2 說「動到本 App 以外的任何檔案要先問」，本 App 是「不准改」（較嚴，照本 App）；本 App STATUS 工作慣例第 1 條的其他專案名單沒有 MealMate，第 11 條與 `CLAUDE.md` 有。
